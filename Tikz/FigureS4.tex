...
\newcommand{\xModelShift}{-15.5}
\newcommand{\yModelShift}{4.5}

\begin{document}
\begin{tikzpicture}[x=\unit, y=\unit]

% Panel a
\node[panel, anchor=north west] (a) at (0cm, 0cm) {\includegraphics[]{"../Supplement figures/SupplementModelRobustness_addNoiseVal"}};
...
% Panel b
\node[panel, anchor=north west] (b) at (a.north east) {\includegraphics{"../Supplement figures/SupplementModelRobustness_mulNoiseVal"}};
...
% Panel c
\node[panel, anchor=north west] (c) at (a.south west) {\includegraphics{"../Supplement figures/SupplementModelRobustness_firstTh"}};
...
% Panel d
\node[panel, anchor=north west] (d) at (c.north east) {\includegraphics{"../Supplement figures/SupplementModelRobustness_secondTh"}};
...
% Panel e
\node[panel, anchor=north west] (e) at (c.south west) {\includegraphics{"../Supplement figures/SupplementModelRobustness_nSubUnit"}};
\begin{scope}[shift={($(e)+(\xModelShift, \yModelShift)$)}]
	\input{Models/ModelFrame}
	\node[box, fill=none, minimum width=\objSize*\unit+2*\pad*\unit, minimum height=\height*\unit, anchor=west, xshift=2*\xSep*\unit - 1*\pad*\unit] (brainBox) at (largeSum.east) {};
	\node[funObj, fill=modelFillColor] (nonLinB) at (brainBox) {};
	\pic[] at (nonLinB) {nonLinFun};
	...
	\draw[edgeShort] (nonLinB.east) --++ (\xSep, 0);
	% Highlight
	\node[anchor=east, inner sep=0.*\unit, fill=white] (nsub) at (subunitCount.east) {\color{discColor} \tinySize n};
\end{scope}
...
\node[label, yshift=1mm] at (e.north west) {\labelSize e};
